...
function Show-EnvVariableManagement {
    do {
        $choice = Show-Menu -Title "环境变量管理" -Options @(
            "返回上级菜单",
            "查看所有环境变量",
            "设置环境变量",
            "删除环境变量"
...
function Show-AllEnvVariables {
    Write-StatusMessage "当前环境变量：" -Type Info
...
function Set-NewEnvVariable {
    ...
    Write-StatusMessage "环境变量 '$name' 已设置为 '$value'" -Type Success
}

function Remove-EnvVariable {
    $name = Read-Host "请输入要删除的环境变量名"
    [Environment]::SetEnvironmentVariable($name, $null, "User")
    Write-StatusMessage "环境变量 '$name' 已删除" -Type Success
}
